...
public class WorkpackageServiceImpl implements WorkpackageService{
    ...
    public Long save(Workpackage workpackage) {
        Long projId = Optional.ofNullable(workpackage.getProject())
                              .map(Project::getId)
                              .orElseThrow(() ->
                                  new ResourceNotFoundException("Workpackage must reference a project"));
    ...
    public Workpackage edit(Workpackage workpackage) {
        ...
        if (wpId == null) {
            throw new ResourceNotFoundException("Workpackage ID must be provided for edit");
        }

        Workpackage existing = workpackageRepository.findById(wpId)
            .orElseThrow(() ->
                new ResourceNotFoundException("Workpackage with id=" + wpId + " not found"));
    ...
    public void delete(Long id) {
        if (!workpackageRepository.existsById(id)) {
            throw new ResourceNotFoundException("Workpackage with id=" + id + " not found");
        }
